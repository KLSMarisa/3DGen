...
def create_final_dataset(csv_path, output_json_path):
    ...
    data = data[non_empty_mask & not_scene_mask].copy()
    ...
    # Take the top 50k high-scoring rows from the filtered dataset
    top_50k = data.sort_values(by="aesthetic", ascending=False).head(25000)
    if len(top_50k) == 0:
        raise ValueError("No rows remain after filtering; cannot build dataset.")
    
    # 输出 top50k 最后一项的美学得分（即第50000名的得分，如果不足50000则输出最后一项得分）
    last_score = top_50k["aesthetic"].iloc[-1]
    print(f"top50k 最后一项的美学得分: {last_score}")
    
    # Randomly sample 50k rows from the filtered dataset
    random_50k = data.sample(n=min(25000, len(data)), random_state=42)
    
    # Combine the top 50k and random 50k rows
    combined_data = pd.concat([top_50k, random_50k]).drop_duplicates().head(50000)
