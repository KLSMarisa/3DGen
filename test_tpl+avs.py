...
def load_obj2render_dataset(
    *,
    ...
    with open('/mnt/hdd3/duqinhong/3dclipscore/code/aesthetic/input/merge_caption.json', 'r') as f:
        data2 = json.load(f)
...
@torch.no_grad()
def intergrate_batch(batch, test=False):
    ...
    # batch['caption'] = ['A three-dimensional model of a yellow sports car with a sleek design and a shiny finish. Positioned at a slight angle, allowing a view of its front and side.']
    # batch['caption'] = ['A fox head with blue eyes, and a black nose.']
    batch['caption'] = ['A 3D animated fox head with a realistic fur texture and a detailed facial expression. A fox has blue eyes, a black nose, and a slight smile.']
    ...
    # caption = 'A fox head with blue eyes, and a black nose.'
    # caption 
    caption = encode_text(caption)
    _batch['caption'] = caption
...
while True:
    try:
        ...
        info = next(data)
        # 处理图像和caption
        _batch, render_gt = intergrate_batch(info)
        B = _batch['images'].shape[0]
        device = _batch['images'].device
        # pimgs = []
        # aimgs = []
        ...
        # log_dict(loss_dict, prog_bar=True, logger=True, on_step=True, on_epoch=True)
                ...
                log_dir = '/mnt/hdd2/caixiao/deeplearning/test/3dgen/tpl_130k+avs_66k/test_github5'
                log_dir = '/mnt/hdd2/caixiao/deeplearning/test/3dgen/iccv/gt23d/test2'
                log_dir = '/mnt/hdd2/caixiao/deeplearning/test/3dgen/iccv/gt23d/test4'
                log_dir = '/mnt/hdd2/caixiao/deeplearning/test/3dgen/iccv/gt23d/fox2'
                # log_dir = '/mnt/hdd2/caixiao/deeplearning/bench/baseline/semv3d/prompt_200'
                # log_dir = '/mnt/hdd2/caixiao/deeplearning/bench/baseline/semv3d/supp1'
                # log_dir = '/mnt/hdd2/caixiao/deeplearning/bench/baseline/semv3d/supp2'
                # log_dir = '/mnt/hdd2/caixiao/deeplearning/bench/baseline/semv3d/supp3'
                # log_dir = '/mnt/hdd2/caixiao/deeplearning/bench/baseline/semv3d/supp4'
                ...
                for i in range(4):
                    ...
                    rgb = to_img(rgb)
                    rgb.save(os.path.join(log_dir, f'{global_step}_{i+4*j}_{part}_{id}_out.png'))
                
            # rgb = ((alpha[i][0]) ).clamp(0, 1).to(torch.uint8).cpu()
            # rgb = to_img(rgb)
            # rgb.save(os.path.join(log_dir, f'{global_step}_{i}_alpha.png'))
            for k in range(3):
                rgb = (_batch['images'][0][k] * 255. ).clamp(0, 255).to(torch.uint8).cpu()
                rgb = to_img(rgb)
                rgb.save(os.path.join(log_dir, f'{global_step}_{part}_{id}_triplane_{k}.png'))
            # mesh_path_idx = os.path.join(log_dir, f'{global_step}.obj')
