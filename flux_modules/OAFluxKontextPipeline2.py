...
# Change relative imports to direct imports from the same directory
from . import OAFluxTransformer2DModel
from . import env_set
import deepspeed
import json
import numpy as np
...
class OAFluxKontextPipeline2(FluxKontextPipeline):
    def prepare_latents(
        ...
        if image is not None:
            ...
            latents = torch.randn((batch_size,image_latent_height*image_latent_width//4,64), device=device, dtype=dtype)
    ...
    def __call__(
        ...
        if transformer is not None:
            ...
            print('transformer has been replaced')
        #print(self.transformer)
        #self.transformer.config.guidance_embeds = False
...
#
        ...
        #print('img resized to: ',image.shape)
        ...
        #print(f"Number of denoising steps: {self._num_timesteps}")
        #print(timesteps)
        # handle guidance
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                print_tensor_info(noise_pred, 'noise_pred')
                if do_true_cfg:
...
def get_pipeline(ckpt_path,run_config, Train = False):
    ...
    if ckpt_path is not None and not ckpt_path.endswith('.bin'):
        ...
        print('trying loading from: ', ckpt_path)
    ...
    if not Train:
        ...
        pipe = OAFluxKontextPipeline2(
            vae=base_pipe.vae,
            text_encoder=base_pipe.text_encoder,
            tokenizer=base_pipe.tokenizer,
            scheduler=base_pipe.scheduler,
            transformer=oa_transformer,  # 使用.module访问实际模型
            text_encoder_2=base_pipe.text_encoder_2,
            tokenizer_2=base_pipe.tokenizer_2,
        )
        print('oa transformer: ',oa_transformer.device)
        print('vae device: ',base_pipe.vae.device)
        print('text encoder device: ',base_pipe.text_encoder.device)
        print('text encoder 2 device: ',base_pipe.text_encoder_2.device)
    else:
        print('loading pipeline for train...')
        pipe = OAFluxKontextPipeline2(
            vae=base_pipe.vae,
            text_encoder=base_pipe.text_encoder,
            tokenizer=base_pipe.tokenizer,
            scheduler=base_pipe.scheduler,
            transformer=oa_transformer, 
            text_encoder_2=base_pipe.text_encoder_2,
            tokenizer_2=base_pipe.tokenizer_2,
        )
