...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, context=None, dataset=None):
        for layer in self:
...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dims == 2:
            ...
            out = rearrange(out, '(b f) c l -> b f l c', b=B, f=F)
        else:
            raise NotImplementedError
        return out
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, datasets=[], y=None, **kwargs):
        ...
        if self.predict_codebook_ids:
            ...
            B, C, F, H, W = h.shape
            h = rearrange(h, 'b c f h w -> (b f) c h w')
            # for name, param in self.out[0].named_parameters():
            #     print(name)
            #     print("0-4",param)
            # print(h.dtype)
            out = self.out(h)
