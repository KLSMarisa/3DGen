...
from modules.StableDiffusion import UNetModel
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb=None, context=None, dataset=None):
        for layer in self:
...
class SptialTriplaneResBlock(TimestepBlock):
    def __init__(self, SpatialResBlock : ResBlock):
        super().__init__()
        # SpatialResBlock = SpatialResBlock
        self.TriplaneResBlock = ResBlock(
            SpatialResBlock.out_channels,
            # SpatialResBlock.emb_channels,
            SpatialResBlock.dropout,
            SpatialResBlock.out_channels,
            SpatialResBlock.use_conv,
            SpatialResBlock.use_scale_shift_norm,
            1,
            SpatialResBlock.use_checkpoint,
            False, False
        )
...
class SpatialTriplaneTransformer(nn.Module):
    def __init__(self, STransformer: SpatialTransformer):
        ...
        self.TTransformer = TriplaneTransformer(
            STransformer.in_channels,
            STransformer.n_heads,
            STransformer.d_head,
            STransformer.depth,
            STransformer.dropout,
            STransformer.context_dim,
            STransformer.disable_self_attn,
            STransformer.use_linear,
            STransformer.use_checkpoint
        )
...
class process_plane_net(nn.Module):
    """
    The sub of UNet model with attention and no timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
    :param out_channels: channels in the output Tensor.
    :param num_res_blocks: number of residual blocks per downsample.
    ...
    def forward(self, x, timesteps=None, context=None, datasets=[], y=None, **kwargs):
        ...
        # h = h.type(x.dtype)
        # print('h',h.shape)
        h = self.middle_block(h, datasets)
        # print('h_mid',h.shape)
        # h = h.type(x.dtype)
