...
ckpt_path = '/mnt/hdd1/model/SD1.5/models/unet/diffusion_pytorch_model_1_1.bin'
...
# ckpt_path = '/mnt/hdd1/caixiao/3dgen_mvsd/stage2_sd-step=55000.ckpt/55k.bin'
# ckpt_path = '/mnt/hdd1/caixiao/3dgen_sd2.1_stage3_fintune/checkpoints/stage2_sd-step=35000.ckpt/35k.bin'
# ckpt_path = '/home/caixiao/projects/3DGen/train/stage3_ctx_finetune/checkpoints/stage2_sd-step=70000-v1.ckpt/70k.bin'
ckpt_path = '/home/caixiao/projects/3DGen/train/zero_snr_finetune/checkpoints/stage2_sd-step=70000.ckpt/70k.bin'
ckpt_path = '/mnt/hdd1/caixiao/deeplearning/ckpt/3dgen/stage1_no_sd/checkpoints/stage1_no_sd-step=3000.ckpt/3k.bin'
...
## 从pipe中导出的schedule，对于pipe而言效果确实提升了不少
...
# #### pipe
# ## ldm3d的pipe
# pipe = StableDiffusionLDM3DPipeline.from_pretrained("/mnt/hdd1/caixiao_hdd1/datasets/l3dm4c", torch_dtype=torch.float16)
...
# ## unet
# unet_P = pipe.unet
# unet_P.load_state_dict(torch.load('/mnt/hdd1/caixiao_hdd1/datasets/l3dm4c/unet/diffusion_pytorch_model.bin',map_location='cpu'))
# unet_P.to("cuda")
...
while True:
    try:
        ...
        # prompt = "a white bust on a black background"
        # text = "a white box with a handle on it"
        # prompt = "a white cannon on a black background"
        # prompt = "a white ship in the dark with a black background"
        # prompt = "a 3d model of a man in a shirt"
        text = info['caption']
        # print(text)
        # print(info['file'])
        # text = "a wooden cart with wheels"
        # text = "a woman wearing a pink shirt and blue jeans"
        # text = "A chair is sitting in front of a table"
        # text = "A blue and grey bench with four sections."
        # text = "A sword with a green handle and a brown grip"
        # text = "A lamp with a blue shade and a white base"
        # text = "A robot with a brown head and orange body"
        # text = "A brown hamburger with a yellow bun and a red meat patty."
        # text = "A purple object with a hole in the center"
          #huggingface主页的prompt
          #huggingface主页的prompt
        # text = "a wooden cart with wheels on a black background"  #huggingface主页的prompt
        # text = "a 3d rendering of a small house in the dark"  #huggingface主页的prompt
        # text = "a black cup with a red rose on it"  #huggingface主页的prompt
        # text = "a yellow chair"  #huggingface主页的prompt
        # text = "a 3d rendering of a roof with a blue light"  #huggingface主页的prompt
        # text = "a 3d model of a duck head"  #huggingface主页的prompt
        # text = "a 3d model of a man with a guitar"  #huggingface主页的prompt
        # text = "a 3d model of a woman with a pink robe"  #huggingface主页的prompt
        # text = "a woman with a pink robe"  #huggingface主页的prompt
        # text = "sonic the hedgehog - tails"  #huggingface主页的prompt
        # text = "a skeleton is standing on a black background"  #huggingface主页的prompt
        # text = "A skeleton holding a sword"  #huggingface主页的prompt
        # text = "3d model of a rock and tree"  #huggingface主页的prompt
        # text = "a dark room with a dark brown wall"  #huggingface主页的prompt
        # text = "A wooden desk with a black monitor on it"  #huggingface主页的prompt
        # text = "A brown wicker trash can with a wooden top"   #huggingface主页的prompt
        # text = "A man wearing a blue shirt and brown pants"   #huggingface主页的prompt
        # text = "A statue of a clock with a ring around it"   #huggingface主页的prompt
        # text = "A wooden cabinet with a grey top and brown drawers"   #huggingface主页的prompt
        # text = "A green chair with a black metal base"   #huggingface主页的prompt
        # text = "A grey knife with a black handle"   #huggingface主页的prompt
        text = "A blue shoe with white stripes on the side"   #huggingface主页的prompt
        # text = "A gold coin is sitting on "   #huggingface主页的prompt
        text = "A flower with a yellow center and a purple petal."   #huggingface主页的prompt
        text = "A sword with a yellow handle."   #huggingface主页的prompt
        text = "A grey chair with a metal frame and a black seat."   #huggingface主页的prompt
        # text = "A white shoe with a blue swoosh on the side."   #huggingface主页的prompt
        text = 'a young beautiful anime girl with long hair, wearing a white dress, anime style, highly detailed'
        ...
        os.makedirs(f"/mnt/hdd1/caixiao/test/3dgen/ablatiaon/no_sd/3k", exist_ok=True)
        ...
        for i in range(3):
            ...
            img_unet.save(f"outputs/{timestamp}_{counter}_{i}.png")
            # exit()
        # depth_unet.save(f"/mnt/nfs/caixiao/test/3DGEN/stage1/step_125000/dep_125000_{counter}_{timestamp}_{text}.png")
        # CUDA_VISIBLE_DEVICES=6 python /home/caixiao/projects/3DGen/test_pipe.py
        # 可输出所有12张图 仅限our unet
        # for i in range(12):
        #     depth_unet = out_unet[0, i, 3, :, :]
        #     img_unet = out_unet[0, i, :3, :, :]
        #
        #     to_img = ToPILImage()
        ...
        #     img_unet = to_img(img_unet)
        #     depth_unet = to_img(depth_unet)
        #     img_unet.save(f"/home/caixiao/projects/3DGen/test_unet/unet{counter}_{i}.png")
        #     depth_unet.save(f"/home/caixiao/projects/3DGen/test_unet/dep_unet{counter}_{i}.png")
