...
class StableDiffusionTrainer(pl.LightningModule):
    def __init__(self, config):
        ...
        # ckpt_path = '/mnt/hdd1/model/StableDiffusion/text_encoder/pytorch_model.bin'
        self.text_clip.load_state_dict(torch.load(ckpt_path, map_location='cpu'), strict=False)
        ...
        if self.stage > 1:
            ...
            # ckpt_path = '/home/caixiao/projects/3DGen/train/pytorch_model.bin'
        ...
        if self.stage > 2:
            ...
            # ckpt_path = '/home/caixiao/projects/3DGen/train/55k.bin'
            ckpt_path = '/mnt/hdd1/caixiao/deeplearning/ckpt/3dgen/stage3_zerosnr_finetune/zero_snr_finetune/checkpoints/stage2_sd-step=70000.ckpt/70k.bin'
            sd = torch.load(ckpt_path, map_location='cpu')
            for k, v in sd.items():
                if '_forward_module.unet.' in k:
                    _sd[k.replace('_forward_module.unet.', '')] = v
            self.unet.load_state_dict(_sd, strict=True)
    ...
    def configure_optimizers(self):
        opt = DeepSpeedCPUAdam(
            ...
            lr=self.config["base_learning_rate"] if self.stage < 3 else self.config["base_learning_rate"]/10, 
            betas=(0.9, 0.9), 
            weight_decay=0.03
        )
    ...
    def intergrate_batch(self, batch):
        ...
        i = 0
        # torch.save(text_latent,f"/mnt/hdd1/caixiao/3dgen/test/text_emb/text_{i}.pt")
        # print(text_latent.shape)
        # exit()
        _batch['text'] = text_latent.to(self.dtype)
        mimage_latent = self.encode_images(batch['rgbd']) # N * C * 3 * H * W
        # mimage_latent = mimage_latent.to(self.dtype) # N * C * 3 * H * W
        # print(mimage_latent.dtype)
        # print(self.dtype)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # print(planes.shape)
        # print(_batch['images_target'].shape)
        loss_images = F.mse_loss(planes.float().flatten(1), _batch['images_target'].float().flatten(1))
