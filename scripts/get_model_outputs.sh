...
for STEP in $(seq 500 500 $MAX_STEP); do
    ...
    python -m torch.distributed.launch --master_port 99  --nproc_per_node 1 --use_env main.py --version $version  --init_step $STEP --exp flux --config configs/base.yaml --mode inference --batch_size 1 --name flux_training  --savedir '/mnt/hdd3/linzhuohang/3DGen/ckpt' --device 1 --nodes 1 --resume '/mnt/hdd3/linzhuohang/3DGen/ckptv'$version'/checkpoints/flux_training-step='$STEP'.ckpt'
done
